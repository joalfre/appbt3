class PostresController < ApplicationController
  ...
  def insertar
    ...
    #Redireccionamos a la vista principal con el mensaje
    @ini = "/postres/index"
  ...
  # Procesamos la actualizacion de un registro en la base de datos
  def editar
    ...
    @ini = "/postres/index"
    flash[:notce] = "Actualizado Correctamente !"
    redirect_to @ini
  end
  ...
  def eliminar
    ...
    # Eliminamos la imagen preteneciente al registro
    simg = Postre.where(:id => @postres).pluck(:img)
    imgeliminar = Rails.root.join('public', 'assets/img', simg.join)
    File.delete(Rails.root + imgeliminar)
    ...
    # Redireccionamos a la vista principal con mensaje
    @inir = "/postres/index"
    flash[:notice] = "Eliminado Corectamente !"
    redirect_to @ini
  end

  # Paramétros o campos que insertamos o actualizamos en la base de datos
  private
  def parametros
